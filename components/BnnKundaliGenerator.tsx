...
import { generateBnnAnalysis, summarizeText, chatWithAnalysis } from '../services/geminiService';
...
interface BnnKundaliGeneratorProps {
  onBack: () => void;
  language: Language;
}

export const BnnKundaliGenerator: React.FC<BnnKundaliGeneratorProps> = ({ onBack, language }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setChatHistory([]);
    try {
      // Fix: Added language argument to generateBnnAnalysis call.
      const analysis = await generateBnnAnalysis(details, language);
      setResult(analysis);
    ...
    <div className="flex flex-col items-center gap-6">
      <div className="w-full p-6 bg-slate-800/50 rounded-lg border border-slate-700 backdrop-blur-sm">
        <div className="flex justify-between items-center mb-6">
          ...
          <h2 className="text-2xl font-semibold text-center font-hindi">Career Analysis (BNN)</h2>
          <div className="w-10"></div>
        </div>
        <p className="text-center text-slate-400 mb-4">Apne career ka vishleshan karne ke liye janam ki jaankari dein.</p>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {isLoading ? <><Spinner /> Analyzing...</> : 'Get Career Analysis'}
          </button>
        </form>
      </div>
      {result && <ResultCard title="Career Analysis Result" content={result} onSummarize={handleSummarize} isSummarizing={isSummarizing} />}
